...
void *ntwrkThrdFunc(void *arg)
{
    ...
    printf("[NETWORK THREAD] TCP server listening on port %d\n", PORT_NUMBER);

    while (1)
    {
        printf("[NETWORK THREAD] Waiting for client...\n");
        ...
        printf("[NETWORK THREAD] Client connected...\n");
        ...
        while (1)
        {
            ...
            hdr.mgc = PROTOCOL_MAGIC;
            hdr.version = PROTOCOL_VERSION;
            hdr.type = MSG_SAMPLES;
            hdr.payload_len = payloadLen;
            hdr.reserved = 0;

            /* Send header */
            if(sendAll(clientFd, &hdr, sizeof(hdr)) < 0)
                break;
            
            /* Send payload */
            if(sendAll(clientFd, batch, payloadLen) < 0)
                break;
        }
